...
import { Search, Settings, User, Plus, LogOut, Edit, Trash2, Shield, Database, Upload, Download, Phone, Mail, Globe, MapPin, UserX, UserCheck } from "lucide-react";
...
export default function Dashboard() {
  ...
  const canEdit = useCallback(() => userProfile?.role === 'Edit' || userProfile?.role === 'Admin', [userProfile?.role]);
  const canDelete = useCallback(() => userProfile?.role === 'Edit' || userProfile?.role === 'Admin', [userProfile?.role]);
  const canAdmin = useCallback(() => userProfile?.role === 'Admin', [userProfile?.role]);
  ...
  useEffect(() => {
    if (selectedContact && contacts.length > 0) {
      const updatedSelectedContact = contacts.find(contact => contact.id === selectedContact.id);
  ...
  const confirmDeactivateUser = async () => {
    ...
    try {
      ...
      showError("Deactivate Failed", "Failed to deactivate user. Please try again.");
    }
  };
  ...
  const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          if (!contact.initials && contact.name) {
            contact.initials = contact.name.split(' ')
              .map((n: string) => n[0])
              .join('')
              .toUpperCase()
              .slice(0, 2);
      ...
      showError("Import Failed", "Failed to import CSV file. Please check the file format.");
    } finally {
  ...
  const exportContactsToCSV = () => {
    try {
      ...
      showError("Export Failed", "Failed to export contacts. Please try again.");
    }
  };
